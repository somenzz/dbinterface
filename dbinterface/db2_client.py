...
class Db2Client(DataBaseInterface):
    ...
    def get_table_info(self,schema,tabname):
        ...
        columns_info = self.dbi_connection.columns(schema_name=schema, table_name=tabname)

        ##print("列信息")
        # print(columns_info)
        cols = []
        for col in columns_info:
            cols.append([col['COLUMN_NAME'],col['REMARKS'],col['TYPE_NAME'],col['COLUMN_SIZE'],col['DECIMAL_DIGITS'],col['IS_NULLABLE']])
        ###print(cols)
        ###print("主键信息")
        primary_keys_info = self.dbi_connection.primary_keys(schema_name=schema, table_name=tabname)
        pks = {}
        ...
        ##print("外键信息")
        fks = {}
        foreign_keys_info = self.dbi_connection.foreign_keys(schema_name=schema, table_name=tabname)
        ...
        for fk in foreign_keys_info :
            if fk['FK_NAME'] in fks:
                ...
                fks[fk['FK_NAME']] = [[fk['FKCOLUMN_NAME']],[fk['PKTABLE_NAME']],[fk['PKCOLUMN_NAME']]]
        ##print(fks)
        ##print("索引信息")
        ids = {}
        indexes_info = self.dbi_connection.indexes(schema_name=schema, unique = True,table_name=tabname)
    ...
    def get_tables(self,schema_name='EDW'):
       ...
       'TABLE_SCHEM': 'SYSCAT', 'TABLE_NAME': 'VARIABLEAUTH', 'TABLE_TYPE': 'VIEW', 'REMARKS': None
        ...
        for table in tables_info:
            if table_type == '' :
                tabs.append([table['TABLE_SCHEM'],table['TABLE_NAME'],table['TABLE_TYPE'],table['REMARKS']])
            else:
